...
(async () => {
  try {
    ...
    const quoteModulePath = path.join(process.cwd(), 'src/lib/quotePdfFile.ts');
  ...
  const { generateQuotePDF } = mod as QuoteMod;
    ...
    const pdfBytes = await generateQuotePDF({
      ...
      items: [
        { name: 'TUERCA HX 2H GALV', qty: 1, price: 1000, subtotal: 1190, sku: 'SKU-123', unit_size: '1/2"', measurement_unit: 'pulg', characteristics: ['Galvanizado'] },
        { name: 'TORNILLO M8x20', qty: 10, price: 120, subtotal: 143, sku: 'SKU-456', unit_size: 'M8', measurement_unit: 'mm', characteristics: ['Acero Inox'] },
      ],
