...
export async function GET(request: NextRequest) {
  try {
    ...
    let base = supabaseAdmin.from('fasercon_suppliers').select('*', { count: 'exact' }).order('name', { ascending: true })
    if (q) {
      const like = `%${q}%`
      base = supabaseAdmin.from('fasercon_suppliers').select('*', { count: 'exact' }).or(`name.ilike.${like},email.ilike.${like}`)
    }
...
export async function POST(request: NextRequest) {
  ...
  if (body.website) contactInfo.website = body.website
    ...
    const { data: existing } = await supabaseAdmin.from('fasercon_suppliers').select('*').ilike('name', String(body.name)).limit(1)
    if (existing && existing.length) return NextResponse.json({ message: 'Proveedor ya existe', supplier: existing[0] })

    const { data, error } = await supabaseAdmin.from('fasercon_suppliers').insert([insert]).select().single()
    if (error) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from('fasercon_suppliers').update(toUpdate).eq('id', id).select().single()
    if (error) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { id } = body
    if (!id) return NextResponse.json({ error: 'Missing id' }, { status: 400 })
    const { error } = await supabaseAdmin.from('fasercon_suppliers').delete().eq('id', id)
    if (error) {
      console.error('Error deleting supplier:', error)
      return NextResponse.json({ error: 'Error al eliminar proveedor' }, { status: 500 })
    }
    // Optionally nullify supplier_id on products that referenced it
    await supabaseAdmin.from('fasercon_products').update({ supplier_id: null }).eq('supplier_id', id)
